...
const Tag = ({ color, children, remove }) => {
  return (
    <span className={`${ colors[color] } inline-flex items-center text-sm rounded mt-2 mr-1 overflow-hidden`}>
      <span className="ml-2 mr-1 leading-relaxed truncate max-w-xs px-1" x-text="tag">{ children }</span>
      { remove &&
        <button className="w-6 h-8 inline-block align-middle text-gray-500 bg-grey-900 bg-opacity-50 focus:outline-none" onClick={ remove }>
