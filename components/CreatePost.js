...
const CreatePost = ({ addPost, channel, parentType, parentId, isReply, visibility }) => {
  ...
  const updateContent = (content) => {
    ...
    setNewPost({ ...newPost, content, tags: filterTags(postTags.concat(manuallyAddedTags)) });
  }

  const postToChannel = async (request) => {
    try {
      const { data: { results: post } } = await api.post('/post', {
        ...request,
        channel,
        parentType,
        parentId,
