  ...
  return (
    <div className="ticket-list-preview card">
      <div className="card-body">
        <p>{ __('ticket_list_id') } <b>{ ticket.get('_id') }</b></p>
        { ticket.get('name') &&
        ...
        { ticket.get('option') &&
          <p>{ __('ticket_list_type') } <b>{ ticket.getIn(['option', 'name']) }</b></p>
        }
        { ticket.get('fields') &&
      ...
      <div className="card-footer">
        <Link href={`/tickets/${ticket.get('_id')}`}><a className="btn">{ __('ticket_list_view_ticket') }</a></Link>
      </div>
    </div>
  );
}
